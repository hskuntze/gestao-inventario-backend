...
@Component
public class TangivelExcelReader implements AtivoExcelReader<Tangivel> {
	...
	@Override
	public List<Tangivel> read(MultipartFile file) throws IOException {
		List<Tangivel> lista = new ArrayList<>();
		...
		try (Workbook workbook = WorkbookFactory.create(file.getInputStream())) {
			...
			for (int i = 1; i <= sheet.getLastRowNum(); i++) {
				...
				t.setEstadoConservacao(getContent(row, 7));
				t.setCodigoSerie(getContent(row, 8));
				...
				t.setObservacoes(getContent(row, 10));
