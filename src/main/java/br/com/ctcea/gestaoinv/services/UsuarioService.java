...
public class UsuarioService implements UserDetailsService {
	...
	public Usuario getAuthenticatedUser() {
		...
		Usuario usuario = usuarioRepository.findByLogin(auth.getName())
				.orElseThrow(() -> new RecursoNaoEncontradoException("Erro ao tentar resgatar usuário logado"));
		return usuario;
	}
	...
	public UsuarioDTO getById(Long id) {
		Usuario u = usuarioRepository.findById(id).orElseThrow(
				() -> new RecursoNaoEncontradoException("Não foi possível localizar um usuário com ID " + id));
	...
	public UsuarioDTO getByLogin(String login) {
		Usuario u = usuarioRepository.findByLogin(login).orElseThrow(
				() -> new RecursoNaoEncontradoException("Não foi possível localizar um usuário com login " + login));
		return new UsuarioDTO(u);
	}
	...
	public UsuarioDTO registrar(UsuarioRegistroDTO dto) {
		if (dto != null) {
			...
			if (aux.isEmpty()) {
				...
				UsuarioResponsavel responsavel = usuarioResponsavelRepository
						.findById(dto.getUsuarioResponsavel().getId())
						.orElseThrow(() -> new RecursoNaoEncontradoException(
								"Não foi possível localizar um usuário responsável com ID "
										+ dto.getUsuarioResponsavel().getId()));
	...
	@Transactional
	public void trocarPrimeiraSenha(String novaSenha) {
		if (novaSenha != null) {
			...
			throw new RequisicaoNaoProcessavelException(
					"Requisição improcessável. Nova senha e/ou senha antiga estão vazias ou nulas.");
		}
	}
		...
		if (novaSenha != null && userId != null) {
			...
			if (isAdmin) {
				Usuario usuario = usuarioRepository.findById(userId).orElseThrow(
						() -> new RecursoNaoEncontradoException("Usuário com ID " + userId + " não foi localizado."));

				usuario.setPassword(passwordEncoder.encode(novaSenha));
				usuarioRepository.save(usuario);
			} else {
				throw new RequisicaoNaoProcessavelException(
						"Acesso negado! Você precisa ser administrador para realizar essa ação. Registrando IP em log.");
			}
		} else {
			throw new RequisicaoNaoProcessavelException(
					"Requisição improcessável. Nome de usuário e/ou senha estão vazias ou nulas.");
		}
	}
